...
app.post('/save-poll-data', async (req, res) => {
    try {
       ...
       console.log("---SETE:::--",dbConfig)
        if (!(dbConfig.database && dbConfig.user && dbConfig.password &&
            dbConfig.port && dbConfig.host)) {
                 console.log("---I M HEYA22 222----")
            return res.status(500).json({ success: false, error: 'Database configuration is incomplete' });
        }
        ...
        while (retries < maxRetries) {
            try {
                ...
                // Handle UUID collision (duplicate key violation)
                if (insertError.code === '23505' && insertError.constraint === 'impilo_sessions_impilo_uid_key') {
                    retries++;
                    if (retries < maxRetries) {
                        // Generate a new UUID and retry
                        console.log(`UUID collision detected. Retrying with new UUID (attempt ${retries}/${maxRetries})`);
                        impiloUid = uuidv4();
                        continue;
                    } else {
